...
async def get_user(user_id: Optional[int] = None, db: Session = Depends(get_db)):
    if user_id is None:
        # If no ID provided, we could return all users (admin only) or just handle as optional
        # Requirement says GET /user/{user_id?}, implying it might be optional.
        # But usually it's for a specific user. 
